...
if (!config.DISCORD_TOKEN) {
    exitWithError(
        "Missing discord token! Make sure to fill out the .env file."
    );
} else if (
    ...
    exitWithError(
        "Missing spotify application information! Make sure to fill out the .env file."
    );
...
function parseSpotifyURLSFromString(inputString) {
    ...
    for (const parsedMessage2 of messageArray) {
        if (parsedMessage2.includes("track/")) {
            const spotifyID = parsedMessage2
                .split("track/")[1]
                .split("?")[0]
                .split("&")[0];
            spotifyIDs.push("spotify:track:" + spotifyID);
        } else if (
            parsedMessage2.length == 22 &&
            /^[\dA-Za-z]+$/.test(parsedMessage2)
        ) {
...
function sendUpdateMessage(user, spotifyIDs) {
    spotify.getInfoForSongs(spotifyIDs).then((nameArray) => {
        ...
        const addedString = `<@${user.id}> added ${nameArray.length} song${
            nameArray.length > 1 ? "s" : ""
        } to the playlist:\n${nameArray
            .map((data) => {
                return `• ${data}`;
            })
        ...
        if (config.ANNOUNCEMENT_CHANNEL) {
            client.channels.cache
                .get(config.ANNOUNCEMENT_CHANNEL)
                .send(addedString);
        }
    });
}
client.once("ready", () => {
    ...
    client.on("message", (message) => {
        ...
        if (
            config.IS_LOCKED_TO_CHANNEL &&
            config.LOCK_CHANNEL &&
            message.channel.id.toString() != config.LOCK_CHANNEL
        )
            return;
        ...
        if (spotifyIDs.length > 0) {
            spotify.getInfoForSongs(spotifyIDs).then((nameArray) => {
                ...
                nameArray = spotifyIDs.filter(
                    (song) => !nameArray.includes(song)
                ); // remove already present songs
                ...
                spotify.AddToPlaylist(nameArray).then(() => {
                    ...
                    message.reply(
                        `Added to playlist! Check it out here: <https://open.spotify.com/playlist/${config.PLAYLIST_ID}>`
                    );
                });
            });
        }
