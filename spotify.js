...
function Authenticate(code) {
    ...
    )}&scope=playlist-read-collaborative%20playlist-modify-public%20playlist-read-private%20playlist-modify-private`;
    if (tokens.refresh != "") {
    ...
    if (!code || code == "") {
        console.log(
            "Spotify client credential not found! To get this, click this link and fill out SPOTIFY_CLIENT_CREDENTIAL with the part that's returned as ?code=XXXXXX"
        );
...
function AddToPlaylist(ids) {
    return new Promise((resolve, reject) => {
        spotifyApi
            .addTracksToPlaylist(config.PLAYLIST_ID, ids)
            .then((data) => {
...
function getFullPlaylistInfo(previousTracks = [], offset = 0) {
    return new Promise((resolve, reject) => {
        spotifyApi
            .getPlaylistTracks(config.PLAYLIST_ID, {
                length: 100,
                offset: offset,
            })
            .then((data) => {
                // console.log(data.body);
                const songItems = data.body.items
                    .map((track) => {
                        return track.track;
                    })
                    .concat(previousTracks);
...
function getInfoForSongs(ids) {
    return new Promise((resolve, reject) => {
        ...
        getFullPlaylistInfo(config.PLAYLIST_ID)
            .then((playlistTracks) => {
                for (const id of ids) {
                    ...
                    if (foundTrack) {
                        songNames.push({
                            id: id,
                            name: stringFromTrackInfo(foundTrack),
                        });
                    }
                }
                resolve(songNames);
